...
class upload {
    ...
    function upload($file, $lang = 'en_GB') {
        ...
        $this->lang               = $lang;
        if ($this->lang != 'en_GB' && file_exists(__DIR__.'/lang') && file_exists(__DIR__.'/lang/class.upload.' . $lang . '.php')) {
            $translation = null;
            include(__DIR__.'/lang/class.upload.' . $lang . '.php');
        ...
        if (!is_array($file)) {
            if (empty($file)) {
                ...
                if (str_starts_with((string) $file, 'php:') || str_starts_with((string) $file, 'data:') || str_starts_with((string) $file, 'base64:')) {
                    $data = null;

                    // this is a PHP stream, i.e.not uploaded
                    if (str_starts_with((string) $file, 'php:')) {
                        $file = preg_replace('/^php:(.*)/i', '$1', (string) $file);
                        if (!$file) $file = $_SERVER['HTTP_X_FILE_NAME'];
                        if (!$file) $file = 'unknown';
                        $data = file_get_contents('php://input');
                        $this->log .= '<b>source is a PHP stream ' . $file . ' of length ' . strlen((string) $data) . '</b><br />';

                    // this is the raw file data, i.e.not uploaded
                    } else if (str_starts_with((string) $file, 'data:')) {
                        $data = preg_replace('/^data:(.*)/i', '$1', (string) $file);
                        $file = 'data';
                        $this->log .= '<b>source is a data string of length ' . strlen((string) $data) . '</b><br />';

                    // this is the raw file data, base64-encoded, i.e.not uploaded
                    } else if (str_starts_with((string) $file, 'base64:')) {
                        $data = base64_decode(preg_replace('/^base64:(?:.*base64,)?(.*)/i', '$1', (string) $file));
                        $file = 'base64';
                        $this->log .= '<b>source is a base64 data string of length ' . strlen((string) $data) . '</b><br />';
                    }
                    ...
                    if ($this->uploaded) {
                        ...
                        preg_match('/\.([^\.]*$)/', (string) $this->file_src_name, $extension);
                        if (is_array($extension) && count($extension) > 0) {
                            $this->file_src_name_ext      = strtolower((string) $extension[1]);
                            $this->file_src_name_body     = substr((string) $this->file_src_name, 0, ((strlen((string) $this->file_src_name) - strlen((string) $this->file_src_name_ext)))-1);
                        } else {
                            $this->file_src_name_ext      = '';
                    ...
                    if ($this->uploaded) {
                        ...
                        $this->file_src_name       = basename((string) $file);
                        $this->log .= '- local file OK<br />';
                        preg_match('/\.([^\.]*$)/', (string) $this->file_src_name, $extension);
                        if (is_array($extension) && count($extension) > 0) {
                            $this->file_src_name_ext      = strtolower((string) $extension[1]);
            ...
            if ($this->uploaded) {
                $this->file_src_error         = trim((string) $file['error']);
        ...
        if ($this->uploaded) {
            ...
            if (!$this->file_src_mime || !is_string($this->file_src_mime) || empty($this->file_src_mime) || !str_contains((string) $this->file_src_mime, '/')) {
                if ($this->mime_fileinfo) {
                    ...
                    if ($this->function_enabled('finfo_open')) {
                        ...
                        if (is_resource($f)) {
                            ...
                            if (preg_match("/^([\.\w-]+)\/([\.\w-]+)(.*)$/i", (string) $this->file_src_mime)) {
                                $this->file_src_mime = preg_replace("/^([\.\w-]+)\/([\.\w-]+)(.*)$/i", '$1/$2', (string) $this->file_src_mime);
                ...
                if ($this->mime_file) {
                    ...
                    if (!str_starts_with(PHP_OS, 'WIN')) {
                        if ($this->function_enabled('exec') && $this->function_enabled('escapeshellarg')) {
                            if (strlen($mime = @exec("file -bi ".escapeshellarg((string) $this->file_src_pathname))) != 0) {
                                $this->file_src_mime = trim((string) $mime);
            ...
            if (!$this->file_src_mime || !is_string($this->file_src_mime) || empty($this->file_src_mime) || !str_contains((string) $this->file_src_mime, '/')) {
                if ($this->mime_getimagesize) {
                    ...
                    if (is_array($info) && array_key_exists('mime', $info)) {
                        $this->file_src_mime = trim((string) $info['mime']);
            ...
            if ($this->file_src_mime == 'application/octet-stream' || !$this->file_src_mime || !is_string($this->file_src_mime) || empty($this->file_src_mime) || !str_contains((string) $this->file_src_mime, '/')) {
    ...
    function gdversion($full = false) {
        ...
        if ($gd_version === null) {
            ...
            if (preg_match($regex, (string) $gd, $m)) {
    ...
    function function_enabled($func) {
        ...
        if ($blacklist === null) $blacklist = extension_loaded('suhosin') ? array_map('trim', array_map('strtolower', explode(',', ini_get('  suhosin.executor.func.blacklist')))) : [];
    ...
    function rmkdir($path, $mode = 0777) {
        return is_dir($path) || ( $this->rmkdir(dirname((string) $path), $mode) && $this->_mkdir($path, $mode) );
    }
    ...
    function temp_dir() {
        ...
        if (!$dir) {
            ...
            if (file_exists($tmp)) {
                unlink($tmp);
                $dir = dirname((string) $tmp);
            }
        }
        if (!$dir) return '';
        $slash = (strtolower(substr(PHP_OS, 0, 3)) === 'win' ? '\\' : '/');
        if (substr((string) $dir, -1) != $slash) $dir .= $slash;
        return $dir;
    }
    ...
    function getcolors($color) {
        ...
        if (strlen((string) $color) == 3) $color = str_repeat(substr((string) $color, 0, 1), 2) . str_repeat(substr((string) $color, 1, 1), 2) . str_repeat(substr((string) $color, 2, 1), 2);
    ...
    function getoffsets($offsets, $x, $y, $round = true, $negative = true) {
        if (!is_array($offsets)) $offsets = explode(' ', (string) $offsets);
        if (count($offsets) == 4) {
             $ct = $offsets[0]; $cr = $offsets[1]; $cb = $offsets[2]; $cl = $offsets[3];
        } else if (count($offsets) == 2) {
            $ct = $offsets[0]; $cr = $offsets[1]; $cb = $offsets[0]; $cl = $offsets[1];
        } else {
            $ct = $offsets[0]; $cr = $offsets[0]; $cb = $offsets[0]; $cl = $offsets[0];
        }
        if (strpos((string) $ct, '%')>0) $ct = $y * (str_replace('%','',$ct) / 100);
        if (strpos((string) $cr, '%')>0) $cr = $x * (str_replace('%','',$cr) / 100);
        if (strpos((string) $cb, '%')>0) $cb = $y * (str_replace('%','',$cb) / 100);
        if (strpos((string) $cl, '%')>0) $cl = $x * (str_replace('%','',$cl) / 100);
        if (strpos((string) $ct, 'px')>0) $ct = str_replace('px','',$ct);
        if (strpos((string) $cr, 'px')>0) $cr = str_replace('px','',$cr);
        if (strpos((string) $cb, 'px')>0) $cb = str_replace('px','',$cb);
        if (strpos((string) $cl, 'px')>0) $cl = str_replace('px','',$cl);
    ...
    function process($server_path = null) {
        ...
        if ($this->processed) {
            if (empty($server_path) || is_null($server_path)) {
                ...
                if(strtolower(substr(PHP_OS, 0, 3)) === 'win') {
                    if (!str_ends_with((string) $server_path, '\\')) $server_path .= '\\';
                } else {
                    if (!str_ends_with((string) $server_path, '/')) $server_path .= '/';
                }
        ...
        if ($this->processed) {
            ...
            if ($this->no_script) {
                ...
                if ((((str_starts_with((string) $this->file_src_mime, 'text/') && $this->file_src_mime != 'text/rtf') || str_contains((string) $this->file_src_mime, 'javascript'))  && (!str_ends_with((string) $file_src_name, '.txt')))
                    || preg_match('/\.(php|php5|php4|php3|phtml|pl|py|cgi|asp|js)$/i', (string) $this->file_src_name)
            ...
            } else if ($this->mime_check && !empty($this->file_src_mime) && str_contains((string) $this->file_src_mime, '/')) {
                [$m1, $m2] = explode('/', (string) $this->file_src_mime);
                $allowed = false;
                // check wether the mime type is allowed
                if (!is_array($this->allowed)) $this->allowed = [$this->allowed];
                foreach($this->allowed as $v) {
                    [$v1, $v2] = explode('/', (string) $v);
        ...
        if ($this->processed) {
            ...
            if (!is_null($this->file_name_body_add)) { // append a string to the name
                $this->file_dst_name_body .= $this->file_name_body_add;
            ...
            if ($this->file_safe_name) { // formats the name
                $this->file_dst_name_body = mb_convert_encoding(strtr(mb_convert_encoding((string) $this->file_dst_name_body, 'ISO-8859-1'), mb_convert_encoding('ŠŽšžŸÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÑÒÓÔÕÖØÙÚÛÜÝàáâãäåçèéêëìíîïñòóôõöøùúûüýÿ', 'ISO-8859-1'), 'SZszYAAAAAACEEEEIIIINOOOOOOUUUUYaaaaaaceeeeiiiinoooooouuuuyy'), 'UTF-8', 'ISO-8859-1');
                $this->file_dst_name_body = strtr($this->file_dst_name_body, ['Þ' => 'TH', 'þ' => 'th', 'Ð' => 'DH', 'ð' => 'dh', 'ß' => 'ss', 'Œ' => 'OE', 'œ' => 'oe', 'Æ' => 'AE', 'æ' => 'ae', 'µ' => 'u']);
                $this->file_dst_name_body = preg_replace(['/\s/', '/\.[\.]+/', '/[^\w_\.\-]/'], ['_', '.', ''], (string) $this->file_dst_name_body);
            ...
            if (!$return_mode) {
                if (!$this->file_auto_rename) {
                    ...
                    if ($file_src_name_ext != $this->file_src_name_ext) {
                        if (substr((string) $this->file_dst_name_body, -1 - strlen((string) $this->file_src_name_ext)) == '.' . $this->file_src_name_ext) {
                            $body = substr((string) $this->file_dst_name_body, 0, strlen((string) $this->file_dst_name_body) - 1 - strlen((string) $this->file_src_name_ext));
        ...
        if ($this->processed) {
            ...
            if (!$return_mode) {
                ...
                $hash = md5($this->file_dst_name_body . random_int(1, 1000));
            ...
            if ($image_manipulation) {
                ...
                if ($this->processed && $image_src) {
                    ...
                    if ($gd_version >= 2 && !empty($auto_flip)) {
                        ...
                        for ($x = 0; $x < $this->image_src_x; $x++) {
                            for ($y = 0; $y < $this->image_src_y; $y++){
                                if (str_contains((string) $auto_flip, 'v')) {
                    ...
                    if ($gd_version >= 2 && is_numeric($auto_rotate)) {
                        if (!in_array($auto_rotate, [0, 90, 180, 270])) $auto_rotate = 0;
                        if ($auto_rotate != 0) {
                    ...
                    if ($this->image_resize) {
                        ...
                        // keeps aspect ratio with x and y dimensions, filling the space
                            ...
                            $this->image_ratio_crop = strtolower((string) $this->image_ratio_crop);
                            if (($this->image_src_x/$this->image_x) > ($this->image_src_y/$this->image_y)) {
                                ...
                                if (str_contains((string) $this->image_ratio_crop, 'l')) {
                                    $ratio_crop['l'] = 0;
                                    $ratio_crop['r'] = $ratio_crop['x'];
                                } else if (str_contains((string) $this->image_ratio_crop, 'r')) {
                                ...
                                if (str_contains((string) $this->image_ratio_crop, 't')) {
                                    $ratio_crop['t'] = 0;
                                    $ratio_crop['b'] = $ratio_crop['y'];
                                } else if (str_contains((string) $this->image_ratio_crop, 'b')) {
                        ...
                        // keeps aspect ratio with x and y dimensions, fitting the image in the space, and coloring the rest
                            ...
                            $this->image_ratio_fill = strtolower((string) $this->image_ratio_fill);
                            if (($this->image_src_x/$this->image_x) < ($this->image_src_y/$this->image_y)) {
                                $this->image_dst_y = $this->image_y;
                                $this->image_dst_x = intval($this->image_src_x*($this->image_y / $this->image_src_y));
                                $ratio_crop = [];
                                $ratio_crop['x'] = $this->image_dst_x - $this->image_x;
                                if (str_contains((string) $this->image_ratio_fill, 'l')) {
                                    $ratio_crop['l'] = 0;
                                    $ratio_crop['r'] = $ratio_crop['x'];
                                } else if (str_contains((string) $this->image_ratio_fill, 'r')) {
                                    $ratio_crop['l'] = $ratio_crop['x'];
                                    $ratio_crop['r'] = 0;
                                } else {
                                    $ratio_crop['l'] = round($ratio_crop['x']/2);
                                ...
                                $ratio_crop = [];
                                $ratio_crop['y'] = $this->image_dst_y - $this->image_y;
                                if (str_contains((string) $this->image_ratio_fill, 't')) {
                                    $ratio_crop['t'] = 0;
                                    $ratio_crop['b'] = $ratio_crop['y'];
                                } else if (str_contains((string) $this->image_ratio_fill, 'b')) {
                                    $ratio_crop['t'] = $ratio_crop['y'];
                                    $ratio_crop['b'] = 0;
                                } else {
                                    $ratio_crop['t'] = round($ratio_crop['y']/2);
                    ...
                    if ($gd_version >= 2 && ($this->image_negative || $this->image_greyscale || is_numeric($this->image_threshold)|| is_numeric($this->image_brightness) || is_numeric($this->image_contrast) || !empty($this->image_tint_color))) {
                        ...
                        if (!empty($this->image_tint_color)) [$tint_red, $tint_green, $tint_blue] = $this->getcolors($this->image_tint_color);
                    ...
                    if ($gd_version >= 2 && !empty($this->image_border)) {
                        ...
                        $this->image_dst_x = $this->image_dst_x + $cl + $cr;
                        $this->image_dst_y = $this->image_dst_y + $ct + $cb;
                        if (!empty($this->image_border_color)) [$red, $green, $blue] = $this->getcolors($this->image_border_color);
                    ...
                    if ($gd_version >= 2 && is_numeric($this->image_frame)) {
                        if (is_array($this->image_frame_colors)) {
                            ...
                            $vars = explode(' ', (string) $this->image_frame_colors);
                        }
                        $nb = count($vars);
                        $this->image_dst_x += $nb * 2;
                        $this->image_dst_y += $nb * 2;
                        $tmp = $this->imagecreatenew($this->image_dst_x, $this->image_dst_y);
                    ...
                    if ($this->image_watermark!='' && file_exists($this->image_watermark)) {
                        ...
                        if ($watermark_checked) {
                            ...
                            if (is_numeric($this->image_watermark_x)) {
                                ...
                                if (str_contains((string) $this->image_watermark_position, 'r')) {
                                    $watermark_x = $this->image_dst_x - $watermark_dst_width;
                                } else if (str_contains((string) $this->image_watermark_position, 'l')) {
                                    $watermark_x = 0;
                                } else {
                            ...
                            if (is_numeric($this->image_watermark_y)) {
                                ...
                                if (str_contains((string) $this->image_watermark_position, 'b')) {
                                    $watermark_y = $this->image_dst_y - $watermark_dst_height;
                                } else if (str_contains((string) $this->image_watermark_position, 't')) {
                                    $watermark_y = 0;
                                } else {
                    ...
                    if (!empty($this->image_text)) {
                        ...
                        if (!is_numeric($this->image_text_font) && strlen((string) $this->image_text_font) > 4 && str_ends_with(strtolower((string) $this->image_text_font), '.gdf')) {
                            if (!str_contains((string) $this->image_text_font, '/')) $this->image_text_font = "./" . $this->image_text_font;
                        ...
                        if (!is_numeric($this->image_text_font) && strlen((string) $this->image_text_font) > 4 && str_ends_with(strtolower((string) $this->image_text_font), '.ttf')) {
                            $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;try to load font ' . $this->image_text_font . '... ';
                            if (!str_contains((string) $this->image_text_font, '/')) $this->image_text_font = "./" . $this->image_text_font;
                        ...
                        // get the text bounding box (TrueType fonts)
                            ...
                            $text_offset_x = 0;
                            $text_offset_y = 0;
                            $rect = imagettfbbox($this->image_text_size, $this->image_text_angle, $this->image_text_font, (string) $text );
                            if ($rect) {
                        ...
                        if (is_numeric($this->image_text_x)) {
                            ...
                            if (str_contains((string) $this->image_text_position, 'r')) {
                                $text_x = $this->image_dst_x - $text_width;
                            } else if (str_contains((string) $this->image_text_position, 'l')) {
                                $text_x = 0;
                            } else {
                        ...
                        if (is_numeric($this->image_text_y)) {
                            ...
                            if (str_contains((string) $this->image_text_position, 'b')) {
                                $text_y = $this->image_dst_y - $text_height;
                            } else if (str_contains((string) $this->image_text_position, 't')) {
                                $text_y = 0;
                            } else {
                                ...
                                foreach ($text as $k => $v) {
                                    if ($this->image_text_direction == 'v') {
                                        imagestringup($filter,
                                                      ...
                                                      $text_height - (2 * $this->image_text_padding_y) - ($this->image_text_alignment == 'l' ? 0 : (($t_height - strlen((string) $v) * $char_width) / ($this->image_text_alignment == 'r' ? 1 : 2))) ,
                                                      (string) $v,
                                                      $text_color);
                                    } else {
                                        imagestring($filter,
                                                    $this->image_text_font,
                                                    ($this->image_text_alignment == 'l' ? 0 : (($t_width - strlen((string) $v) * $char_width) / ($this->image_text_alignment == 'r' ? 1 : 2))),
                                ...
                                foreach ($text as $k => $v) {
                                    if ($this->image_text_direction == 'v') {
                                        imagestringup($image_dst,
                                                      ...
                                                      $text_y + $text_height - (2 * $this->image_text_padding_y) - ($this->image_text_alignment == 'l' ? 0 : (($t_height - strlen((string) $v) * $char_width) / ($this->image_text_alignment == 'r' ? 1 : 2))),
                                                      (string) $v,
                                                      $text_color);
                                    } else {
                                        imagestring($image_dst,
                                                    $this->image_text_font,
                                                    $text_x + ($this->image_text_alignment == 'l' ? 0 : (($t_width - strlen((string) $v) * $char_width) / ($this->image_text_alignment == 'r' ? 1 : 2))),
                    ...
                    if ($this->image_reflection_height) {
                        ...
                        if (strpos((string) $image_reflection_height, '%')>0) $image_reflection_height = $this->image_dst_y * (str_replace('%','',$image_reflection_height / 100));
                        if (strpos((string) $image_reflection_height, 'px')>0) $image_reflection_height = str_replace('px','',$image_reflection_height);
                        ...
                        for ($y = 0; $y < $image_reflection_height; $y++) {
                            ...
                            if ($transparency > 0) $transparency -= $this->image_reflection_opacity / $image_reflection_height;
                        }
        ...
        while ($Y >= 0) {
            ...
            while ($X < $bmp['width']) {
                if ($bmp['bits_per_pixel'] == 24)
                    $color = unpack("V",substr((string) $im,$P,3).$vide);
                elseif ($bmp['bits_per_pixel'] == 16) {
                    $color = unpack("n",substr((string) $im,$P,2));
                    $color[1] = $palette[$color[1]+1];
                } elseif ($bmp['bits_per_pixel'] == 8) {
                    $color = unpack("n",$vide.substr((string) $im,$P,1));
                    $color[1] = $palette[$color[1]+1];
                } elseif ($bmp['bits_per_pixel'] == 4) {
                    $color = unpack("n",$vide.substr((string) $im,floor($P),1));
                    if (($P*2)%2 == 0) $color[1] >>= 4 ; else $color[1] &= 0x0F;
                    $color[1] = $palette[$color[1]+1];
                } elseif ($bmp['bits_per_pixel'] == 1)  {
                    $color = unpack("n",$vide.substr((string) $im,floor($P),1));
                    if     (($P*8)%8 == 0) $color[1] >>= 7;
