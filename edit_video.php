...
if (isset($_POST["change_image"])) {
    ...
    $Uploader->process("u/thmp/");

    $Uploader = new upload($_FILES["image"]);
    $Uploader->file_new_name_body      = $_VIDEO->URL."_m";
    $Uploader->image_resize            = true;
    $Uploader->file_overwrite          = true;
    $Uploader->image_x                 = 480;
    $Uploader->image_y                 = 360;
    $Uploader->image_background_color  = '#000000';
    $Uploader->image_convert           = 'jpg';
    $Uploader->image_ratio_fill        = false;
    $Uploader->file_max_size           = 1000000;
...
if (isset($_POST["video_submit"])) {
    ...
    if ($Validation) {
        ...
        $_VIDEO->change_info([
            ...
            "e_comments"     => $Validation["e_comments"],
            "e_ratings"     => $Validation["e_ratings"],
            "category"    => $Validation["category"],
            "date_recorded" => $Date,
            "address"    => $Validation["address"],
            "country"    => $Validation["country"]
        ]);
        notification($LANGS['changessaved'],"/edit_video?v=".$_GET["v"],"cfeeb2"); exit();
    }
}

$Day = (int)substr((string) $_VIDEO->Info['date_recorded'], 8);
$Month = (int)substr((string) $_VIDEO->Info['date_recorded'], 5, -3);
$Year = (int)substr((string) $_VIDEO->Info['date_recorded'], 0, -6);
$CheckCountry = $_VIDEO->Info["country"];

if (isset($_GET["analytics"])) {
    $Daily_Views = $DB->execute("SELECT SUM(views) as views, submit_date FROM views_day WHERE url = :URL GROUP BY DATE(submit_date)", false, [":URL" => $_VIDEO->URL]);
}
...
if ($Videos::$Videos) {
    $Videos = $Videos->fix_values(false,true);
    ...
    if ($Videos_Amount > 1) {
        ...
        $Related_Tags = array_splice($Related_Tags,-6);
        shuffle($Related_Tags);
    }
}

if (isset($_POST["update_video_file"]) && !empty($_FILES["video_file"]["name"]) && $_USER->Info['is_partner'] == 1) {
    ...
    if ($Video_Size <= 2048 && in_array(strtolower($Video_Type), $Allowed_Types)) {
            ...
            if (move_uploaded_file($Video_TMP, $target_path)) {
                ...
                notification("Video successfully updated! It can take 6 hours for your new video to appear due to cache.", "/edit_video?v=".$_GET['v'], "cfeeb2"); exit();
            } else {
                header("location: /");
                exit();
            }
